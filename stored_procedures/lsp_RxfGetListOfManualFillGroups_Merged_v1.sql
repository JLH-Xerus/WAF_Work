...
Begin
   ...
   */
   ...
   -- Index to support hub filtering lookups
   ...
   Begin
       ...
       Begin
           ...
           -- NoLock added for consistency with the rest of the procedure.
           Select
               [NdcInvPool] = Ndc + '-' + Convert(varchar(30), IsNull(InvPool, 0))
           Into #NdcInvPoolInclusionList
           From InvStationNdcAssoc With (NoLock)
           Where StationId = @LocalStationId;
       End
   End

   -- Default ToteId to the tote currently at the specified station if none was passed in
   ...
   -- Create index after #OeDataUserLocs population
   ...
   -- One row per pharmacy, giving us the correct WeekDayCode to filter on
   ...
   Where
       ...
       And ((@LocalBatchFillId Is Not Null And B.BatchFillId = @LocalBatchFillId) Or @LocalBatchFillId Is Null)
       And
       (
   ...
   With
      ...
      , GroupingCriteria As
        (
           -- Set-based replacement for scalar UDF dbo.fOrdGroupingCritValForDisplay.
           -- The scalar UDF executed row-by-row and blocked parallelism for the
           -- entire surrounding query. This version computes GroupingCriteriaVal
           -- plus any split-code suffix once per distinct GroupNum in one pass.
           Select
               DG.GroupNum,
               G.GroupingCriteriaVal
