ALTER PROCEDURE "dbo"."lsp_DbDeleteOldRxData" 
...
Declare @NumOfRowsDeleted Int = 0      -- Number of rows that have been deleted.  For this SP, this is OeOrderHistory rows, specifically.
...
Exec lsp_DbLogSqlEvent 'A', 'DelOldRxData Beg', '', @EvtNotes, 'lsp_DbDeleteOldRxData'
...
Begin Try
   ...
   ------------------------------------------------------------------------------------------
   While Exists (Select * From OeOrderHistory Where HistoryDtTm < @CutoffDtTm) And @NumOfRowsDeleted < @MaxToDelete
      Begin
      ...
      End  -- While Exists (Select * From OeOrderHistory Where HistoryDtTm < @CutoffDtTm) And @NumOfRowsDeleted < @MaxToDelete
   ...
   ---------------------------------------------------------------------------------
   While Exists (Select * From OeOrderAcceptReject Where RejectDtTm < @CutoffDtTm) And @NumOfRowsDeleted < @MaxToDelete
      Begin
   ...
   Set @EvtNotes = ''
   Exec lsp_DbLogSqlEvent 'A', 'DelOldRxData End', 'Successful', @EvtNotes, 'lsp_DbDeleteOldRxData'
...
Begin Catch
   ...
   Exec dbo.lsp_DbLogSqlError 'DelOldRxData Err', @ErrNum, @ErrMsg, 'lsp_DbDeleteOldRxData'
   ...
   Exec lsp_DbLogSqlEvent 'A', 'DelOldRxData End', 'Failed', @EvtNotes, 'lsp_DbDeleteOldRxData'
