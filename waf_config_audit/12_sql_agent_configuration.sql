...
END TRY
BEGIN CATCH
    SELECT [section] = N'04 - SQL Agent jobs',
           [note]    = N'Skipped: ' + ERROR_MESSAGE() + N' (needs SELECT on msdb job tables, e.g. SQLAgentReaderRole).';
END CATCH
...
END TRY
BEGIN CATCH
    SELECT [section] = N'05 - Failed job steps (last 7 days)',
           [note]    = N'Skipped: ' + ERROR_MESSAGE();
END CATCH
...
END TRY
BEGIN CATCH
    SELECT [section] = N'08 - Database Mail profiles',
           [note]    = N'Skipped: ' + ERROR_MESSAGE() + N' (needs DatabaseMailUserRole on msdb).';
END CATCH
