...
These views typically exist for migration or partitioning reasons. The application was designed to eventually split data across tables (for example, moving from varchar IDs to int IDs), and the view provides a stable interface so application code doesn't need to change.
...
UNION ALL views amplify other performance problems:

- Combined with [[LEFT JOIN OR Anti-Pattern]]: the optimizer must consider both view branches against both join paths, which is four access paths.
- Combined with correlated subqueries (see [[Correlated Subqueries to CTEs]]): each subquery invocation expands into two table accesses.
...
In lsp_ImgGetListOfTopXImagesToMove, the `vImgImage` view over `ImgImage` (10.5M rows) and `ImgImage_IntId` (0 rows) was the base of a query with the [[LEFT JOIN OR Anti-Pattern]]. The combination of view expansion plus LEFT JOINs plus OR predicates produced 1,141,351 average logical reads per execution. The materialize-once + EXISTS refactoring (Fix #2 in [[LEFT JOIN OR Anti-Pattern]]) kept `vImgImage` in place but reshaped the surrounding query to push predicates through the view cleanly. Reads dropped to about 214K per execution.
