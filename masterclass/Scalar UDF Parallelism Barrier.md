...
- SET STATISTICS TIME ON will show CPU time at or above elapsed time, which signals serial execution. In a parallel plan, elapsed is below CPU because multiple cores share the work.
...
1. STRING_AGG replaces the cursor-style `@Suffix = Coalesce(@Suffix + '; ', '') + Descrip` concatenation. STRING_AGG is set-based and available in SQL Server 2017 and later.
...
When elapsed is below CPU, you know multiple cores are working. The CPU total goes up slightly (coordination overhead) but wall clock time drops dramatically.

## SQL Server 2019+ Scalar UDF Inlining

SQL Server 2019 introduced automatic scalar UDF inlining under compatibility level 150. The optimizer can sometimes inline the UDF itself without code changes. There are caveats, though:
...
- [[Query Store Triage]]: high CPU time with CPU at or above elapsed is a signal to look for scalar UDFs.
