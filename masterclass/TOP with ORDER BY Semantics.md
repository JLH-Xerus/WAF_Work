...
SQL Server is free to ignore the ORDER BY because a table (even a temp table) is an unordered set by definition. The optimizer may honor the ORDER BY if a suitable index makes it free, but it isn't guaranteed.
...
When TOP is present, the ORDER BY defines the selection criteria. It is the rule for which rows qualify for the top N. Changing the ORDER BY changes which rows are returned. This is not about the physical order of the result. It is about which rows exist in the result at all.
...
TOP combined with ORDER BY creates a rowgoal optimization. The optimizer knows it only needs N rows in a specific order, so it may:
