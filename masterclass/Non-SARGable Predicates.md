...
- Check the Predicate property on the Scan operator. If it contains function calls, that is the culprit.
...
When the IsNull pattern is comparing two columns in the same table (not a parameter), and both can be NULL, there is no clean SARGable equivalent. The fix is typically a computed column with an index:
...
But this is an index and schema change, not a query change. In our refactoring work, we note these patterns but don't change them when the semantic difference matters.

## Real-World Example

In lsp_RxfGetListOfManualFillGroups (v48 to v49), we identified `IsNull(O.PriCodeSys, 0) = IsNull(P.PriCodeSys, 0)` as non-SARGable. We documented it in the changelog under "Noted, NOT Changed" because replacing it with a plain `=` comparison would change the NULL-handling semantics, potentially breaking the business logic for orders or pharmacies with NULL priority codes.

## The Pragmatic Approach

Not every non-SARGable predicate needs to be fixed:

1. If the table is small (under 10,000 rows), a scan is fine. Don't over-optimize.
