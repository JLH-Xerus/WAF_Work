ALTER PROCEDURE "SmartShelf"."lsp_DbDeleteOldSmartShelfErrors" 
...
Object Name:
   lsp_DbDeleteOldSmartShelfErrors

Version:
   1

Description:
   Delete all Errors (i.e., SmartShelf.EvtErrorLog Rows) older than a specified date.
...
Declare @Rc Int                        -- SQL Command Return Code.  (Saved Value of @@Error.)
      ...
      , @Routine VarChar(50)           -- Name of the Stored Procedure (lsp_DbDeleteOldSmartShelfErrors)

Set @Routine = 'lsp_DbDeleteOldSmartShelfErrors'
...
Set @PrintMsg = Concat('Deleting SmartShelf.EvtErrorLog events older than ', Convert(VarChar(30), @CutoffDtTm, 121), '...(', Convert(VarChar(30), GetDate(), 120), ')')
Print @PrintMsg
...
Exec lsp_DbLogSqlEvent 'A', 'DelOldSSErrs Beg', '', @EvtNotes, @Routine
...
Begin Try
   ...
   ---------------------------------------------------------------------------------
   While Exists(Select * From SmartShelf.EvtErrorLog Where DtTm < @CutoffDtTm) And @NumOfRowsDeleted > @MaxToDelete
   Begin
      -----------------------------------
...
Begin Catch
   ...
   Exec dbo.lsp_DbLogSqlError 'DelOldSSErrs Err', @ErrNum, @ErrMsg, @Routine
   ...
   Exec lsp_DbLogSqlEvent 'A', 'DelOldSSErrs End', 'Failed', @EvtNotes, @Routine
